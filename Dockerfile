...
RUN export S6_OVERLAY_ROOTFS_DIR=/usr/local/src/s6-overlay-rootfs/ \
	&& mkdir $S6_OVERLAY_ROOTFS_DIR \
	&& for TARBALL in s6-overlay-noarch.tar.xz s6-overlay-x86_64.tar.xz s6-overlay-symlinks-noarch.tar.xz s6-overlay-symlinks-arch.tar.xz; do curl -fsSL https://github.com/just-containers/s6-overlay/releases/download/v$S6_OVERLAY_VERSION/$TARBALL | tar -xpJ -C $S6_OVERLAY_ROOTFS_DIR; done
...
COPY --from=build /usr/local/src/s6-overlay-rootfs/ /
